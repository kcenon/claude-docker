...
# Cygwin). These environments produce Windows-flavored paths in .env that
# docker-compose.yml's volume expansion cannot resolve correctly.
case "$(uname -s)" in
    MINGW*|MSYS*|CYGWIN*)
...
collect_configuration() {
    ...
    if [[ "$AUTH_PATH" == "B" ]]; then
        ...
        if [[ -z "${API_KEYS[0]}" ]]; then
            log_error "At least the first API key is required for Path B."
            exit 1
        fi
        log_success "API keys collected ($NUM_ACCOUNTS accounts)"
    fi
