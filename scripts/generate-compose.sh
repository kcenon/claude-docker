...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *) echo "Error: unknown generator argument" >&2; exit 1 ;;
    esac
done
if [[ "$OUTPUT_DIR" == "$PROJECT_ROOT" && -z "${CLAUDE_DOCKER_LOCK_TOKEN:-}" ]]; then
    host_policy run-locked -- bash "$SCRIPT_DIR/generate-compose.sh" --env-file "$ENV_FILE" --output-dir "$OUTPUT_DIR"
    exit $?
fi
export CLAUDE_DOCKER_ENV_FILE="$ENV_FILE"
...
# Validate aliases and resource inputs before opening any output file.
host_policy validate-input --env-file "$ENV_FILE" >/dev/null
mkdir -p "$OUTPUT_DIR"
ISOLATED_TMPFS_LINES="$(host_policy scratch --env-file "$ENV_FILE")"
...
emit_account_volumes() {
    ...
    if [[ "$mode" == worktree ]]; then
        echo '      - ${PROJECT_DIR}/.git:/git-common'
        echo "      - \${HOME}/${RT_STATE_DIR}/account-${letter}/.container-worktree.git:${project_target}/.git:ro"
    fi
...
emit_account_environment() {
    ...
    # exist.
    echo "      - CLAUDE_ALLOW_DEGRADED_SETTINGS=\${CLAUDE_ALLOW_DEGRADED_SETTINGS:-}"
    ...
    # Arbitrary host UIDs cannot write the image-owned /home/node even when
    # the root filesystem is writable. Keep global Git config in account state
    # in every mode; isolated mode additionally makes the image read-only.
...
generate_base() {
    local outfile="$OUTPUT_DIR/docker-compose.yml"
    {
...
generate_worktree() {
    local outfile="$OUTPUT_DIR/docker-compose.worktree.yml"
    {
...
generate_isolated() {
    local outfile="$OUTPUT_DIR/docker-compose.isolated.yml"
    {
...
generate_linux() {
    local outfile="$OUTPUT_DIR/docker-compose.linux.yml"
    {
