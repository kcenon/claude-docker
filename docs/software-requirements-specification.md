...
| `REDIS_URL` | compose | Phase 5 only | `redis://:$REDIS_PASSWORD@redis:6379` | URL | Redis connection URL with password (SRS-8.1.7, SRS-8.1.10) |
...
| `WORKER_AUTH_TOKEN` | .env | Phase 6 only | — | 64-char hex | Bearer token for worker HTTP API authentication (SRS-8.2.17); auto-generated by `install.sh` |
| `REDIS_PASSWORD` | .env | Phase 6 only | — | 64-char hex | Redis `--requirepass` password (SRS-8.1.10); auto-generated by `install.sh` |
| `WORKER_PERSONA` | compose | Phase 7 only | — | String | Full persona system prompt injected into worker (SRS-8.6.2) |
...
| 8.1–8.4 Orchestration | SRS-8.1.1–11, SRS-8.2.1–18, SRS-8.3.1–7, SRS-8.4.1–5 | FR-18–24 | G9, G10 | 5, 6 |
...
| — (worker Bearer auth) | SRS-8.2.17–18 | `POST /task` without `Authorization: Bearer <token>` returns HTTP 401; startup log shows warning if `WORKER_AUTH_TOKEN` unset |
| — (Redis password auth) | SRS-8.1.10 | `redis-cli -u redis://redis:6379 PING` without `-a` fails with NOAUTH; `redis-cli -u redis://:$REDIS_PASSWORD@redis:6379 PING` returns PONG |
| — (internal network) | SRS-8.1.11 | `docker network inspect orchestration-internal` shows `"Internal": true`; host cannot connect to Redis port 6379 directly |
