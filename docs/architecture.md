...
Host
└── Docker Compose
    ├── orchestration-internal network (internal: true — no outbound)
    │   ├── redis              (shared context store, redis:7-alpine, --requirepass)
    ...
    └── [claude-a, claude-b from base — still available on default network]
```

### Communication Flow

```
Manager ──HTTP POST (Bearer token)──> Worker-N:9000/task
                       │
...
- **No docker.sock**: All communication via `orchestration-internal` network; no container needs Docker API access
- **Bearer token auth**: Manager passes `WORKER_AUTH_TOKEN` on every `POST /task`; worker uses `crypto.timingSafeEqual` to validate
- **Redis password**: `REDIS_PASSWORD` auto-generated by `install.sh`; all clients connect via `redis://:$PASS@redis:6379`
- **Internal network isolation**: `orchestration-internal` has `internal: true`; Redis and worker ports are not reachable from the host
...
| Risk | Mitigation |
|------|-----------|
| API keys in environment | Use `.env` file (gitignored, mode 0600); never embed in Dockerfile |
| Container escape | Do NOT mount `/var/run/docker.sock` |
| Credential exposure | State directories are mode 0700; `.credentials.json` files are mode 0600; bind mount only what's needed |
| Network over-exposure | Use DevContainer firewall script to whitelist outbound; orchestration traffic on `orchestration-internal` network with `internal: true` (no outbound) |
| Worker API unauthorized access | `WORKER_AUTH_TOKEN` Bearer authentication with timing-safe comparison; auto-generated by `install.sh` |
| Redis unauthorized access | `REDIS_PASSWORD` via `--requirepass`; auto-generated by `install.sh` |
| Concurrent file corruption | Use Tier B (worktrees) for active dual editing |

For a detailed threat analysis see [docs/threat-model.md](threat-model.md).
For secret rotation procedures see [docs/key-rotation.md](key-rotation.md).
